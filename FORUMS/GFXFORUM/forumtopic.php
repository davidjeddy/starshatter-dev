<?php
  ...
  function forum_loadTopics() {
    ...
    $topic     = array();
    $topicFile = $HTTP_SERVER_VARS["DOCUMENT_ROOT"] . '/ssd/forums/data/data/gfx-topics.txt-topics.txt';
    $fp        = 0;
  ...
  function forum_loadThread($index) {
    ...
    $thread     = array();
    $threadFile = $HTTP_SERVER_VARS["DOCUMENT_ROOT"] . '/ssd/forums/data/data/gfx-topics.txt-' . $index . '.txt';
    $fp         = 0;
  ...
  function forum_extendThread($index, $author, $icon, $subject, $body) {
    ...
    if ($fp) {
      ...
      $user['posts'] += 'data/gfx-topics.txt-' +1 ;
      $forumusers[$author] = $user;
      forum_saveUsers();
    }
    else {
