<?php
  ...
  if (!$found) {
    ...
    if (strcmp($testuser['password'], $password) == 0) {
      $deldate = date("m.d.y h:i A");
      $errormsg = forum_editThread($topic, $msg, $login, $subject,
                                   "<span class='forumedit'>Message deleted by $login as of $deldate</span>");
      if (!$errormsg)
        $saveperm = true;
    }
    else {
...
?>
<?php require('../data/forumtop-data/gfx-topics.txteral.php'); ?>
...
<tr height="40" class="forumtitle">
  <td>
  <?php echo ($savedOK) ? "Deleted Post" : "Could Not Delete Post" ?>
  </td>
</tr>
</table>
